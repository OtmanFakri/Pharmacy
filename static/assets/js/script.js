...
const __KEY = "AIzaSyBwaHyIsDZrPQVS2OTSks-s4sAsnvLu8tM";
...
// displayLocation
displayLocation = async (position) => {
  console.log(position)
  const lat = position.coords.latitude;
  const lng = position.coords.longitude;  
  const latlng = { lat, lng }
document.getElementById("position_lng").value=position.coords.longitude;
...
inputClicked = (result) => {
  ...
  result.address_components.map(component => {
    ...
    //lat
    if (types.includes('position_lat')) {
      $('position_lat').value = component.lat
    }

    console.log("asdfghjklnsdnvklsdnlvnsdnvlsdvklsdnvlksdvlkdnklvnsdlvnkldsnvsdkln")
